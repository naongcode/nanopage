...
- 차별화 컨셉 (피하고 싶은 이미지): ${project.differentiation_concept}
...
5. ⭐ 모든 시나리오에서 "${project.project_name}"을 직접 언급하고 구체적으로 묘사하세요. 일반적인 배경 묘사가 아닌, 제품이 주인공인 장면을 만드세요.
...
4. ⭐ "${project.project_name}"을 모든 prompt_text에서 구체적으로 묘사하세요. "농장 풍경"이 아닌 "복숭아가 주렁주렁 열린 과수원"처럼요.`;
}
...
async function generateScenarioBatch(
  ...
  const batchPrompt = buildBatchPrompt(batch, project, marketingConcept, shootingConceptGuide);
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        const hasImages = project.product_images && project.product_images.length > 0;
        const totalSteps = hasImages ? 4 : 3;
        let currentStep = 0;
        ...
        if (hasImages) {
          ...
          try {
            sendProgress(`이미지 ${project.product_images.length}장 분석 중...`);
            const imagePromises = project.product_images.map(async (imageUrl: string) => {
...
`
        ...
        const shootingConceptInstruction = project.shooting_concept
          ? `
## ⭐ 촬영 컨셉 (반드시 반영해야 할 핵심 장면들) ⭐
사용자가 선택한 촬영 컨셉: ${project.shooting_concept}

중요: 위 촬영 컨셉은 반드시 required_scene에 구체적으로 반영되어야 합니다.
- "농장 수확" → "${project.project_name}"이 실제로 수확되는 장면 (예: 나무에 주렁주렁 열린 모습, 농부가 수확하는 모습)
- "포장 공정" → "${project.project_name}"이 위생적으로 포장되는 공정 장면
- "재료 클로즈업" → "${project.project_name}"의 신선한 단면이나 질감이 보이는 클로즈업
- "요리 장면" → "${project.project_name}"을 활용한 요리 조리 장면
일반적인 "농장 풍경"이 아니라, 제품인 "${project.project_name}"이 직접 등장하는 구체적인 장면이어야 합니다.
`
          : '';
...

{
  ...
  "required_scene_1": "필수 연출 상황 1 (촬영 컨셉에서 선택한 장면을 ${project.project_name}와 함께 구체적으로 묘사)",
  "required_scene_2": "필수 연출 상황 2 (촬영 컨셉에서 선택한 다른 장면을 ${project.project_name}와 함께 구체적으로 묘사)",
...
4. 구체적이고 실용적으로 작성하세요
5. ⭐ 촬영 컨셉이 있다면, required_scene에 제품명 "${project.project_name}"을 직접 언급하며 구체적으로 반영하세요 ⭐
`;
        ...
        if (updateError) {
          throw new Error(`마케팅 컨셉 저장 실패: ${updateError.message}`);
        }

        // Step: 시나리오를 5개씩 3배치로 생성
        const shootingConceptGuide = project.shooting_concept
          ? `
## ⭐⭐⭐ 촬영 컨셉 (반드시 시나리오에 반영) ⭐⭐⭐
사용자가 선택한 촬영 컨셉: ${project.shooting_concept}

이 촬영 컨셉은 시나리오에 직접적으로 반영되어야 합니다.
"${project.project_name}"이 실제로 등장하는 구체적인 장면으로 작성하세요:
...
반드시 "${project.project_name}"이 화면의 주인공으로 구체적으로 묘사되어야 합니다.
`
          : '';
        ...
        const batchResults = await Promise.all(
          batches.map((batch) =>
            generateScenarioBatch(batch, project, marketingConcept, shootingConceptGuide)
          )
        );
        ...
        sendEvent({
          type: 'complete',
          data: { project_id: projectId, scenarios: savedScenarios },
        });
        controller.close();
      } catch (error) {
