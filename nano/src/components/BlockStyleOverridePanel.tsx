      ...
      <div className="space-y-3 p-4 bg-gray-50 rounded-lg">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              블록 높이 (px)
              {localStyle.blockHeight === undefined && (
                <span className="ml-2 text-xs text-gray-500">
                  (공통: {commonSettings.blockHeight || 'auto'})
                </span>
              )}
            </label>
            <div className="flex gap-2">
              <input
                type="number"
                value={localStyle.blockHeight ?? ''}
                placeholder={`공통 설정: ${commonSettings.blockHeight || 'auto'}`}
                onChange={(e) =>
                  setLocalStyle({
                    ...localStyle,
                    blockHeight: e.target.value ? parseInt(e.target.value) : undefined,
                  })
                }
              ...
              {localStyle.blockHeight !== undefined && (
                <button
                  onClick={() => {
                    const { blockHeight, ...rest } = localStyle;
                    setLocalStyle(rest);
                  }}
